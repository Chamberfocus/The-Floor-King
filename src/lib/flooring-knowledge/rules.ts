...
export function knowledgeHelpFor(
  ...
  if (q.kind === "cuts") {
    if (q.config?.category === "vinyl") {
      return "Sheet vinyl is roll goods. These cuts are the order quantity — converting room square feet into yards is not a layout and is not billed as an order.";
    }
    ...
    return "Cuts are the order quantity. Converting room square feet into yards is not a cut plan and is not billed as an order. Carpet tile hides the cut list and uses measured area instead. Install labor uses this question's Settings $/sq yd — it does not invent $6.";
  }
  if (key === "tile_layout") {
