...
export function knowledgeHelpFor(
  ...
  if (q.kind === "cuts") {
    ...
    return "Cuts are the order quantity. Converting room square feet into yards is not a cut plan and is not billed as an order. Width starts empty unless the catalog has roll_width_ft. 12'/15' chips are one tap — we do not plant 12'. Carpet tile hides the cut list and uses measured area instead. Install labor uses this question's Settings $/sq yd — it does not invent $6.";
  }
  if (key === "tile_layout") {
