...
import { fetchArAging, todayIso } from "@/lib/data/accounting-reports";
...
export default async function ArAgingPage({
  ...
  const report = await fetchArAging(asOf);
    ...
    <div className="mx-auto max-w-5xl space-y-6 p-4 md:p-6">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight">
          {REPORT_LABELS.arAging}
        </h1>
        <p className="mt-1 text-sm text-muted-foreground">
          Open AR as of {asOf} (historical when RPC applied; current open AR in
          TS fallback).
        </p>
      </div>
      <BooksStatusBanner
      ...
      />
      <ReportDateForm mode="asOf" asOf={asOf} actionPath="/accounting/ar-aging" />
